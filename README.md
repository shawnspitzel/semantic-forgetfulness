...
Current LLMs treat the context window as a flat buffer. When it fills, old content is truncated regardless of importance. As context grows, attention dilutes and critical information gets buried. We call this **context rot**.

Existing approaches (KV pruning, sliding windows, static compression) reduce memory footprint, but none of them learn what a given session needs, instead applying fixed policies to a dynamic problem.
